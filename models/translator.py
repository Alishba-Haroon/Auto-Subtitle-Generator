...
HUGGINGFACE_TOKEN = os.getenv("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")
...
SUPPORTED_LANGUAGES = {
    ...
    "ur": "facebook/mbart-large-50-many-to-many-mmt",  # Urdu
}
    ...
    for lang, model_name in SUPPORTED_LANGUAGES.items():
        if model_name and lang not in translation_pipelines:
            try:
                ...
                # Special handling for MBART models (Urdu and Portuguese)
                if lang in ["ur", "pt"]:
...
def translate_segments(segments: List[Dict], target_lang: str) -> List[Dict]:
    ...
    try:
        ...
        # Special handling for MBART models
    ...
    try:
        ...
        (
            ...
            .output(
                ...
                vf=f"subtitles='{escape_path(srt_path)}'",  # Quotes for paths with spaces
                acodec="copy",
                vcodec="libx264",
                crf=23,
                preset="fast"
